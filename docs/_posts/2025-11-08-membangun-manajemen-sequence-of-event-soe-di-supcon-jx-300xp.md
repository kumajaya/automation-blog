...
<figure style="text-align:center;">
  <div class="mermaid" style="max-width:75%; margin:auto; font-size:0.85rem;">
  %% Diagram Sequence of Event (SOE) di Supcon JX-300
  flowchart TD
      subgraph PACKING["Packing Digital Input"]
          style PACKING fill:#E8F5E9,stroke:#43A047,stroke-width:2px
          DI1[DI 1-16] --&gt; P1[K_16BitsToWord]
          DI2[DI 17-32] --&gt; P2[K_16BitsToWord]
          P1 &amp; P2 --&gt; MERGE[K_2WordToDWord]
      end
      ...
      MERGE &amp; EP --&gt; SOE[K_SOE32]
      RESET[Reset] --&gt; SOE
      SOE --&gt; DELTA[K_SOE32Delta]
      DELTA --&gt; SORT[K_SOE32Sort]
...
Mengonversi waktu sistem Supcon menjadi epoch detik sejak 1 Januari 2000 UTC.</p>
<p><strong>Alur Logika:</strong></p>
<ul>
<li>Validasi input waktu (tahun ≥2000, bulan 1–12, dll).</li>
<li>Hitung jumlah hari sejak 2000 dengan memperhitungkan leap year.</li>
<li>Tambahkan jam, menit, detik → hasil akhir epoch.</li>
<li>Jika <code>Enable=FALSE</code> atau input invalid → output 0.</li>
</ul>
<p><strong>Nilai Audit/Operator:</strong></p>
<ul>
<li>Konsistensi waktu antar‑platform.</li>
<li>Output 0 mudah dikenali sebagai error.</li>
</ul>
...
<p><strong>Tujuan:</strong><br>
Packing 16 input BOOL scalar menjadi satu WORD bitmask.</p>
<p><strong>Alur Logika:</strong></p>
<ul>
<li>Salin input ke array internal.</li>
<li>Loop 0–15 → jika TRUE, set bit sesuai indeks.</li>
<li>Output berupa bitmask 0x0000–0xFFFF.</li>
</ul>
<p><strong>Nilai Audit/Operator:</strong></p>
<ul>
<li>Snapshot 16 kanal dalam satu kata.</li>
<li>Mapping jelas (IN1=bit0, IN16=bit15).</li>
</ul>
<hr>
<h2 id="3-k2wordtodword">3. K_2WordToDWord</h2>
<p><strong>Tujuan:</strong><br>
Menggabungkan dua WORD (Hi, Lo) menjadi satu DWORD.</p>
<p><strong>Alur Logika:</strong></p>
<ul>
<li>Konversi ke ULONG.</li>
<li>Geser Hi 16 bit ke kiri.</li>
<li>Gabungkan dengan Lo menggunakan OR.</li>
<li>Output = Hi&lt;&lt;16 + Lo.</li>
</ul>
<p><strong>Nilai Audit/Operator:</strong></p>
<ul>
<li>Memudahkan pencatatan data panjang (timestamp, ID).</li>
<li>Transparan, mudah ditelusuri.</li>
</ul>
<hr>
<h2 id="4-ksoe32">4. K_SOE32</h2>
<p><strong>Tujuan:</strong><br>
Inti SOE: menangani 32 kanal digital, melatching trip, menyimpan timestamp, mendeteksi first‑out.</p>
<p><strong>Alur Logika:</strong></p>
<ul>
<li>Restore state dari input persisten.</li>
<li>Rebuild first trip jika TripOut invalid.</li>
<li>Reset manual → kosongkan semua latch.</li>
<li>Loop kanal: deteksi perubahan, simpan timestamp, set TripOut jika belum ada.</li>
<li>Map array timestamp ke struct output.</li>
<li>TripTs = timestamp kanal TripOut.</li>
</ul>
<p><strong>Nilai Audit/Operator:</strong></p>
<ul>
<li>Setiap trip dicatat dengan timestamp unik.</li>
<li>TripOut dan TripTs langsung menunjukkan kanal pertama dan waktunya.</li>
</ul>
<hr>
